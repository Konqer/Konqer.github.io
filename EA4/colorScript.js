 var canvas = document.getElementById('canvas');
  ...
  const linesColorApple = {r: 0.4, g: 0.7, b: 0.8, a: 1};

  const triangleColorApple = {r: 0.7, g: 0.5, b: 0.8, a: 1};
  ...
  const linesColorTorus = {r: 0.5, g: 0.7, b: 0.9, a: 1};

  const trianglesColorTorus = {r: 0.8, g: 0.5, b: 0.6, a: 1};
  ...
  function createAppleVertexData() {
    ...
    for (let u = rangeU.min, i = 0; i <= n; i++, u += du) {
      for (let v = rangeV.min, j = 0; j <= m; j++, v += dv) {
        ...
        var y = Math.sin(u) * ( 4 + 3.8 * Math.cos(v))*1;
        var z = (Math.cos(v) + Math.sin(v)-1) * (1+Math.sin(v)) * Math.log(1-Math.PI * v/10 )  + 7.5 * Math.sin(v)*1;
